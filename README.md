...
2. Add META-INF/persistence.xml to your classpath.  You can start with an in memory database by using [derby](https://github.com/dbadia/sqrl-server-base/blob/master/jpa-examples/derby/persistence.xml).  When you ready to use a real DB, here is the [ddl](https://github.com/dbadia/sqrl-server-base/blob/master/sqrl.ddl) and [mysql](https://github.com/dbadia/sqrl-server-base/blob/master/jpa-examples/mysql/persistence.xml) persistence.xml.  Other databases can be supported simply by editing persistence.xml accordingly.
