...
2. Add META-INF/persistence.xml to your classpath.  You can start with an in memory database by using [derby](https://github.com/dbadia/sqrl-server-base/blob/master/jpa-examples/derby/persistence.xml).  When you ready to use a real DB, here is the [ddl](https://github.com/dbadia/sqrl-server-base/blob/master/sqrl.ddl) and [mysql](https://github.com/dbadia/sqrl-server-base/blob/master/jpa-examples/mysql/persistence.xml) persistence.xml
2. Create a `com.github.dbadia.sqrl.server.SQRLConfig` bean and set the required fields accordingly (see javadoc).  This can be done via Spring, JAXB, etc.
* In your application code, you can now create a `com.github.dbadia.sqrl.server.SqrlServerOperations` object using the `SqrlConfig` and `com.github.dbadia.sqrl.server.data.SqrlJpaPersistenceAdapter` objects
