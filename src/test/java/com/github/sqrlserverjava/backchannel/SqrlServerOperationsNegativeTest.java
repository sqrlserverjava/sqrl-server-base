...
public class SqrlServerOperationsNegativeTest {
	...
	// TODO: update this test case for each Nut type
	@Before
	...
	public void testNoiptestNotSentAndIpChanged_QueryCall() throws Throwable {
		...
		final String serverParam = "cXJsOi8vc3FybGphdmEudGVjaC9zcXJsZXhhbXBsZS9zcXJsYmM_bnV0PWVCbms4d3hyQ2RTX3VBMUwzX013Z3cmc2ZuPWMzRnliR3BoZG1FdWRHVmphQSZjb3I9alVKVlVJcEZXQ1AyUEVNZ2l2Q0lFbWUzZDMyR1ZIM1VUYWZ2QW1MMVVxZw";

		final SqrlPersistence sqrlPersistence = TestCaseUtil.setupIdk(idk, correlatorFromServerParam, serverParam);
		...
		final String rawQueryParams = "client=dmVyPTENCmNtZD1xdWVyeQ0KaWRrPW00NzBGYjhPM1hZOHhBcWxOMnBDTDBTb2txUFlOYXp3ZGM1c1Q2U0xuVU0NCm9wdD1zdWsNCg"
				...
				+ "&ids=ROkIkpNyMrUsaD_Y6JIioE1shQ18ddM7b_PWQ5xmtkjdiZ1NtOTri-zOpSj1qptmNjCuKfG-Cpll3tgF1dqvBg";
		// Emulate the login page generation
		final String firstIp = "11.11.11.11";
	...
	public void testNoiptestNotSentAndIpChanged_IdentCall() throws Throwable {
		...
		final String rawQueryParams = "client=dmVyPTENCmNtZD1xdWVyeQ0KaWRrPW00NzBGYjhPM1hZOHhBcWxOMnBDTDBTb2txUFlOYXp3ZGM1c1Q2U0xuVU0NCm9wdD1zdWsNCg"
				...
				+ "&ids=ROkIkpNyMrUsaD_Y6JIioE1shQ18ddM7b_PWQ5xmtkjdiZ1NtOTri-zOpSj1qptmNjCuKfG-Cpll3tgF1dqvBg";
		// Emulate the login page generation
		final String firstIp = "141.119.117.205";
		final MockHttpServletRequest queryRequest = new MockSqrlHttpRequestBuilder(sqrlRequestUrl)
				.withQueryParams(rawQueryParams).fromIP(firstIp).build();
		MockHttpServletResponse servletResponse = new MockHttpServletResponse();
		...
		final String differentIp = "22.22.22.22";
		...
		final SqrlTifResponse expectedSqrlTif = new SqrlTifResponseBuilder(false).addFlag(SqrlTifFlag.COMMAND_FAILED).createTif();
